...
namespace CRM_WindowsForms.Presentation
{
    public partial class CreateOrderStatus : Form
    {
        ...
        private bool ValidateInput()
        {
            ...
            if (ContainsSqlInjectionRisk(orderStatus))
            {
        ...
        private async void createOrderStatusSubmitButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                await executeor.ExecuteNonQueryAsync("[dbo].[spCreateOrderStatus]", parameters);

                MessageBox.Show("New Order Status added successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);

                this.Close();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Failed to add new Order Status: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}