CREATE PROCEDURE [dbo].[CreateBulkBooking]
...
--Book 80% of the economy class capacity
SET @amountOfBookingsToCreate = (
SELECT CAST(ROUND(A.[Economy_Class_Seat_Count] * 0.8, 0) AS INT) AS [Bookings_To_Create] 
FROM [dbo].[Airplane] A
...
--Book 80% of the premium economy class capacity
SET @amountOfBookingsToCreate = (
SELECT CAST(ROUND(A.[Premium_Economy_Class_Seat_Count] * 0.8, 0) AS INT) AS [Bookings_To_Create]
FROM [dbo].[Airplane] A
INNER JOIN @flightsToBook FTB ON A.[Airplane_Id] = FTB.[Airplane_Id]
WHERE FTB.[Flight_Schedule_Id] = @flightScheduleIdCounter
...
SET @ticketTypeToBook = (
SELECT [Ticket_Type_Id]
FROM [dbo].[TicketType] WHERE [Ticket_Type] = 'Premium Economy Class')
...
SET @amountOfBookingsToCreate = (
SELECT CAST(ROUND(A.[Business_Class_Seat_Count] * 0.8, 0) AS INT) AS [Bookings_To_Create]
FROM [dbo].[Airplane] A 
INNER JOIN @flightsToBook FTB ON A.[Airplane_Id] = FTB.[Airplane_Id]
WHERE FTB.[Flight_Schedule_Id] = @flightScheduleIdCounter
)
